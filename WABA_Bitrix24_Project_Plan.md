...
| FR-17 | Contact Now cover image | `contact_now` template header is an image; backend uses the project's Drive cover image if one exists, otherwise falls back to a fixed default brand cover image |
...
- Hosting target/server for the Node.js backend behind `waba-bitrix.premierchoiceint.online` (VM vs. Render/Railway vs. other) — domain is confirmed, hosting provider still to be decided.
